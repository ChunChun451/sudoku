...
# --- *** FIX 1: Find all .c files in the current folder *** ---
...
all:
    $(MAKE) $(MAKEFILE_PARAMS) # <-- THIS IS LINE 375. Make sure it starts with a TAB, not spaces.

# Project target defined by PROJECT_NAME
$(PROJECT_NAME): $(OBJS)
    $(CC) -o $(PROJECT_NAME)$(EXT) $(OBJS) $(CFLAGS) $(INCLUDE_PATHS) $(LDFLAGS) $(LDLIBS) -D$(PLATFORM) # <-- THIS IS LINE 379. Make sure it starts with a TAB.

# Compile source files
# NOTE: This pattern will compile every module defined on $(OBJS)
# --- *** FIX 2: Use the simple compile rule *** ---
%.o: %.c
#$(OBJ_DIR)/%.o: $(SRC_DIR)/%.c
# --- *** END FIX 2 *** ---
    $(CC) -c $< -o $@ $(CFLAGS) $(INCLUDE_PATHS) -D$(PLATFORM) # <-- THIS IS LINE 401. Make sure it starts with a TAB.
